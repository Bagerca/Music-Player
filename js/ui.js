...
export function updateVolumeIcon(volume) {
    const iconContainer = document.getElementById('volumeIcon');
    const muteBtn = document.getElementById('muteBtn'); // Находим кнопку
    ...
    if (volume == 0) {
        ...
        // УБИРАЕМ КЛАСС ЕСЛИ ГРОМКОСТЬ ЕСТЬ
    ...
    // Вставляем новый путь
    iconContainer.innerHTML = `<path fill="currentColor" d="${d}"/>`;
}
