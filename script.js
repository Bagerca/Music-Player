document.addEventListener('DOMContentLoaded', function() {
    ...
    let sparkParticles = [], energySurgeActive = false, energySurgeIntensity = 0;
    let edgeGlowElements = {}, edgeGlowIntensity = 0;
    let lastWaveTime = 0; // Таймер для волн
    ...
    // --- VISUALIZER & LOGIC ---
    function spawnCornerShockwaves(intensity) {
        if (isLiteMode) return;
        
        const corners = ['top-left', 'top-right', 'bottom-left', 'bottom-right'];
        
        // --- НАСТРОЙКА ПАРАМЕТРОВ ВОЛНЫ (МЯГКИЙ РЕЖИМ) ---
        const scale = 20 + (intensity * 40); 
        const duration = 1.5; 
        const thickness = 1 + (intensity * 4);
        ...
        corners.forEach(corner => {
            ...
            if (emitter) {
                ...
                wave.style.setProperty('--target-scale', scale);
                wave.style.setProperty('--wave-duration', `${duration}s`);
                wave.style.setProperty('--wave-thickness', `${thickness}px`);
    ...
    function renderTrackList() {
        ...
        currentTracks.forEach((track, index) => {
            ...
            if (isUserPlaylist) {
                ...
                dragHandleHTML = `
                    <div class="drag-handle" title="Переместить">
                ...
                dragHandleHTML = `<div style="width: 10px;"></div>`;
            }
            ...
            trackItem.innerHTML = `
                ...
                <button class="track-menu-btn" id="menu-btn-${index}">
    ...
    function addDragEvents(item, index) {
        item.addEventListener('dragstart', (e) => {
            ...
            e.dataTransfer.effectAllowed = 'move'; e.dataTransfer.setData('text/html', item.innerHTML);
        });

        item.addEventListener('dragend', () => {
            item.classList.remove('dragging'); draggedItemIndex = null;
            document.querySelectorAll('.track-item').forEach(el => { el.classList.remove('drag-over-top'); el.classList.remove('drag-over-bottom'); });
        });
        ...
        item.addEventListener('dragover', (e) => {
            ...
            item.classList.remove('drag-over-top'); item.classList.remove('drag-over-bottom');
            if (offset < rect.height / 2) { item.classList.add('drag-over-top'); } else { item.classList.add('drag-over-bottom'); }
        });
        ...
        item.addEventListener('drop', (e) => {
            ...
            const [draggedTrack] = currentTracks.splice(draggedItemIndex, 1);
            
            if (draggedItemIndex < targetIndex) { targetIndex--; }
    ...
    function analyzeEdgeEffects(features) { if (currentTracks.length === 0) return; const { isBeat } = features; const now = Date.now(); if (isBeat) { if (now - lastWaveTime > 400) { if (currentPulseIntensity > 0.75) { spawnCornerShockwaves(currentPulseIntensity); lastWaveTime = now; } } if (!energySurgeActive && currentPulseIntensity > 0.6) { const intensity = 0.5 + currentPulseIntensity * 0.4; activateEnergySurge(intensity); } } }
