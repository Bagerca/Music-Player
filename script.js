document.addEventListener('DOMContentLoaded', function() {
    ...
    let uploadedTracks = []; 
    let pendingUploadFile = null; // Файл который мы редактируем
    ...
    // Управление треками внутри плейлиста
    function moveTrack(index, direction) {
        if (!userPlaylists[currentPlaylistName]) return;
        const newIndex = index + direction;
        if (newIndex < 0 || newIndex >= currentTracks.length) return;
        
        // Меняем местами
        const temp = currentTracks[index];
        currentTracks[index] = currentTracks[newIndex];
        currentTracks[newIndex] = temp;
    ...
    function removeTrack(index) {
        ...
        if (confirm('Убрать этот трек из плейлиста?')) {
            ...
            saveUserPlaylists();
            renderTrackList();
            // Если удалили текущий трек, может потребоваться логика остановки, но пока оставим просто ререндер
        }
    }
    ...
    function saveTrackFromEditor() {
        ...
        const newTrack = {
            ...
            colors: { 
                ...
                secondary: adjustColorBrightness(editColorPrimary.value, -20), // Чуть темнее основного
    ...
    function initializePlayer() {
        ...
        switchPlaylist(currentPlaylistName); // Обновит кнопки удаления и тд
    }
    ...
    function renderTrackList() {
        ...
        currentTracks.forEach((track, index) => {
            ...
            trackItem.addEventListener('click', (e) => {
                if (!e.target.closest('.add-to-playlist-btn') && !e.target.closest('.track-control-btn')) {
                    loadTrack(index, true);
                }
            });

            // КНОПКИ УПРАВЛЕНИЯ (ТОЛЬКО ДЛЯ ПОЛЬЗОВАТЕЛЬСКИХ ПЛЕЙЛИСТОВ)
            if (userPlaylists[currentPlaylistName]) {
                ...
                if (index > 0) {
                    ...
                    upBtn.className = 'track-control-btn';
                    upBtn.innerHTML = '↑';
                ...
                if (index < currentTracks.length - 1) {
                    ...
                    downBtn.className = 'track-control-btn';
                    downBtn.innerHTML = '↓';
                ...
                delBtn.className = 'track-control-btn delete';
                delBtn.innerHTML = '✕';
            ...
            // Кнопка добавления (+)
            // Показываем если есть плейлисты и это НЕ текущий пользовательский плейлист (чтобы не дублировать внутри него)
    ...
    function updateTheme() { 
        ...
        // ДИНАМИЧЕСКАЯ ПОКРАСКА БОКОВОЙ ПАНЕЛИ
    ...
    // Хелпер для прозрачности hex цвета
    ...
    function switchPlaylist(playlistName) { 
        ...
        if (allPls[playlistName]) { 
            ...
            // Показать кнопку удаления плейлиста только для пользовательских
