document.addEventListener('DOMContentLoaded', function() {
    ...
    // ==========================================
    // 2. ВСПОМОГАТЕЛЬНЫЕ ФУНКЦИИ (HELPER FUNCTIONS)
    // ==========================================
    ...
    // Преобразование HEX цвета в RGBA с прозрачностью
    function adjustColorOpacity(hex, opacity) {
        let r=0, g=0, b=0;
        if (!hex) return `rgba(255, 255, 255, ${opacity})`; // Защита от undefined
    ...
    // Изменение яркость HEX цвета (для генерации вторичных цветов)
    ...
    function loadTrack(index, autoPlay = false) { 
        if (currentTracks && index >= 0 && index < currentTracks.length) { 
            ...
            // ОБНОВЛЕНИЕ ТЕМЫ ВЫЗЫВАЕТСЯ ЗДЕСЬ
            updateTheme(); 
    ...
    function updateTheme() { 
        ...
        // ЗДЕСЬ ИСПОЛЬЗУЕТСЯ adjustColorOpacity
