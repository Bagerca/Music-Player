document.addEventListener('DOMContentLoaded', function() {
    ...
    function renderTrackList() {
        ...
        currentTracks.forEach((track, index) => {
            ...
            if (isUserPlaylist) { 
                trackItem.querySelector('.remove-track').addEventListener('click', (e) => { e.stopPropagation(); removeTrack(index); closeAllMenus(); }); 
            }
    ...
    function loadTrack(index, autoPlay = false) { 
        if (currentTracks && index >= 0 && index < currentTracks.length) { 
            ...
            const onLoaded = function() { 
                ...
                if (autoPlay) { setTimeout(() => playTrack(), 100); } 
            }; 
    ...
    function updateProgress() { 
        if (audio.duration && !isNaN(audio.duration)) { 
            ...
            // ЛОГИКА СУБТИТРОВ ПЕРЕНЕСЕНА В VISUALIZE ДЛЯ ПЛАВНОСТИ
        } 
    }
    ...
    function activateEnergySurge(intensity, type = 'normal') { energySurgeActive = true; energySurgeIntensity = intensity; if (type === 'fast') currentSurgeDecay = 0.15; else if (type === 'slow') currentSurgeDecay = 0.02; else currentSurgeDecay = 0.08; const waves = [ document.getElementById('energyTop'), document.getElementById('energyRight'), document.getElementById('energyBottom'), document.getElementById('energyLeft') ]; const currentColors = currentTracks[currentTrackIndex].colors; waves.forEach(wave => { wave.style.opacity = intensity.toString(); const shadowSize = 10 + (intensity * 40); wave.style.background = `linear-gradient(${ wave.classList.contains('top') || wave.classList.contains('bottom') ? '90deg' : '180deg' }, transparent, ${currentColors.accent}, transparent)`; wave.style.boxShadow = `0 0 ${shadowSize}px ${currentColors.accent}`; }); }
    function updateEnergySurge() { if (energySurgeActive && energySurgeIntensity > 0) { energySurgeIntensity -= currentSurgeDecay; if (energySurgeIntensity < 0) { energySurgeIntensity = 0; energySurgeActive = false; } const waves = [ document.getElementById('energyTop'), document.getElementById('energyRight'), document.getElementById('energyBottom'), document.getElementById('energyLeft') ]; waves.forEach(wave => { wave.style.opacity = energySurgeIntensity.toString(); }); } }
    ...
    function analyzeEdgeEffects(features) { if (currentTracks.length === 0) return; const { rms, bassEnergy, isBeat, highEnergy } = features; if (rms < 0.15) { if (!energySurgeActive) { const breatheIntensity = 0.1 + (highEnergy * 0.2); activateEnergySurge(breatheIntensity, 'slow'); } } else { if (isBeat) { let surgePower = (bassEnergy * 0.6) + (rms * 0.6); if (surgePower > 1) surgePower = 1; if (surgePower > 0.65) { spawnCornerShockwaves(surgePower); } if (!energySurgeActive) { const decayType = highEnergy > 0.25 ? 'fast' : 'normal'; activateEnergySurge(surgePower, decayType); } } } }
    ...
    function spawnCornerShockwaves(intensity = 1) { if (isLiteMode) return; const allCorners = ['top-left', 'top-right', 'bottom-left', 'bottom-right']; let activeCorners = []; if (intensity > 0.8) { activeCorners = allCorners; } else { activeCorners = allCorners.sort(() => 0.5 - Math.random()).slice(0, 2); } activeCorners.forEach(corner => { const emitter = document.querySelector(`.corner-emitter.${corner}`); if (emitter) { const wave = document.createElement('div'); wave.className = 'shockwave active'; if (currentTracks.length > 0) { wave.style.borderColor = currentTracks[currentTrackIndex].colors.accent; } wave.style.borderWidth = `${2 + (intensity * 6)}px`; wave.style.opacity = (intensity * 0.7).toString(); emitter.appendChild(wave); setTimeout(() => { if (wave.parentNode) wave.parentNode.removeChild(wave); }, 800); } }); }
    function toggleLiteMode() { isLiteMode = !isLiteMode; localStorage.setItem('isLiteMode', isLiteMode); const sparkParticlesContainer = document.getElementById('sparkParticles'); if (isLiteMode) { sparkParticlesContainer.innerHTML = ''; document.body.classList.add('lite-mode'); liteModeBtn.classList.add('active'); } else { document.body.classList.remove('lite-mode'); liteModeBtn.classList.remove('active'); } }
    function switchPlaylist(playlistName) { const allPls = getAllPlaylists(); if (allPls[playlistName]) { currentPlaylistName = playlistName; currentTracks = allPls[playlistName]; if (userPlaylists[playlistName]) { deletePlaylistBtn.style.display = 'flex'; } else { deletePlaylistBtn.style.display = 'none'; } const isCurrentTrackInNewPlaylist = currentTracks.some(track => audio.src.includes(track.path)); if (!isCurrentTrackInNewPlaylist) { audio.pause(); isPlaying = false; playPauseBtn.querySelector('svg').innerHTML = '<path d="M8 5v14l11-7z"/>'; if (animationId) cancelAnimationFrame(animationId); if (currentTracks.length === 0) { progress.style.width = '0%'; currentTime.textContent = '0:00'; } loadTrack(0); } renderTrackList(); } }
