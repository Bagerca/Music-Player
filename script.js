document.addEventListener('DOMContentLoaded', function() {
    ...
    function spawnCornerShockwaves(intensity) {
        ...
        const scale = 10 + (intensity * 50); 
        const duration = 1.2 - (intensity * 0.8); 
        const thickness = 2 + (intensity * 8);
        ...
        corners.forEach(corner => {
            ...
            if (emitter) {
                ...
                setTimeout(() => { if (wave.parentNode) wave.parentNode.removeChild(wave); }, duration * 1000 + 100);
            }
        });
    }
    ...
    function visualize() {
        ...
        try {
            ...
            updateEnergySurge(); updateEdgeGlow(audioFeatures); updatePulseIntensity();
    ...
    function analyzeEdgeEffects(features) { if (currentTracks.length === 0) return; const { isBeat } = features; if (isBeat) { if (currentPulseIntensity > 0.5) { spawnCornerShockwaves(currentPulseIntensity); } if (!energySurgeActive) { const intensity = 0.5 + currentPulseIntensity * 0.4; activateEnergySurge(intensity); } } }
